...
class PmbNonRegProdiChart extends StatefulWidget {
  String fakKode;
  PmbNonRegProdiChart({Key? key, required this.fakKode}) : super(key: key);

  @override
  State<PmbNonRegProdiChart> createState() =>
      _PmbNonRegProdiChartState(this.fakKode);
}

class _PmbNonRegProdiChartState extends State<PmbNonRegProdiChart> {
  String fakKode;
  _PmbNonRegProdiChartState(this.fakKode);
  @override
  Widget build(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: BlocBuilder<AkademikCubit, AkademikState>(
        bloc: akdCubit..getPmbNonRegulerProdi(fakKode),
        buildWhen: (previous, current) => current is PmbJalurNonRegState,
        builder: (context, state) {
          // print('page prodi');
          // print(fakKode);
          // print(state);
          if (state is PmbJalurNonRegLoaded) {
